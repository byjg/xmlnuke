	...
	public class ProcessPageStateDB : ProcessPageStateBase
	{
		...
		protected virtual DbParameter preProcessValue(string fieldName, INPUTTYPE dataType, string currentValue)
		{
			...
			else if ((dataType == INPUTTYPE.DATE) || (dataType == INPUTTYPE.DATETIME))
			{
				...
				result.Value = com.xmlnuke.util.DateUtil.ConvertDate(currentValue, this._dateFormat);
				if (dataType == INPUTTYPE.DATETIME)
				{
					string[] arCurrentValue = currentValue.Split('|');
					if (arCurrentValue.Length > 1)
					{
						result.Value += " " + arCurrentValue[1];
					}
				}
			}
			else
		...
		{
			DBDataSet dbdata = new DBDataSet(this._conn, this._context);
			...
			SQLHelper helper = new SQLHelper(dbdata);
			...
			dbdata.execSQL(retdata.SQL, retdata.Parameters);
		}
