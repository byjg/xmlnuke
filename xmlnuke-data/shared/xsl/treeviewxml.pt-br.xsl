...
-->
  ...
  <xsl:template match="/">
    <html>
      <head>
        <title><xsl:value-of select="$xml"/>.<xsl:value-of select="$lang"/>.xml output</title>
        <link type="text/css" rel="stylesheet" href="common/styles/treeviewxml.css"/>
      </head>
      <body>
        <h3><xsl:value-of select="$xml"/>.<xsl:value-of select="$lang"/>.xml output</h3>
