...
else
{
	...
	if ( file_exists($PHPDIR) )
	{
		...
		if ( file_exists($HOME) )
		{
			...
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/imagevalidate.php",  "$HOME/") );
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/xmlnukeadmin.php", "$HOME/") );
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/xmlnuke.inc.php", "$HOME/") );
			//$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/check_install.php.dist", "$HOME/check_install.php") );
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/index.php.dist", "$HOME/index.php") );
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/xmlnuke.php", "$HOME/") );

			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/writepage.inc.php.dist", "$HOME/writepage.inc.php") );
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/unittest.php", "$HOME/") );
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/webservice.php", "$HOME/") );
			$gitIgnore[] = executeShell( "ln -sf", array("$PHPDIR/chart.php", "$HOME/") );

			$gitIgnore[] = "config.inc.php";
			$gitIgnore[] = "# Xmlnuke Files - End";
			$gitIgnore[] = "";

			writeToFile("$HOME/.gitignore", $gitIgnore);
			...
			while ( $i < $argc )
			{
				...
				writeToFile( "$HOME/data/xml/" . $argv[$i] . "/index.php.btree", "xmlnuke\n+home." . $argv[$i] . ".xml" );
				$i++;
			}
			...
			mkdir( "$HOME/lib" );
			writeTemplate( "$DATADIR/sites/_includelist.php.template", "$HOME/lib/_includelist.php", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );

			mkdir( "$HOME/lib/modules" );
			writeTemplate( "$DATADIR/sites/module.php.template", "$HOME/lib/modules/home.class.php", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );

			mkdir( "$HOME/lib/base" );
			writeTemplate( "$DATADIR/sites/adminbasemodule.php.template", "$HOME/lib/base/${PROJECT_FILE}adminbasemodule.class.php", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );
			writeTemplate( "$DATADIR/sites/basedbaccess.php.template", "$HOME/lib/base/${PROJECT_FILE}basedbaccess.class.php", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );
			writeTemplate( "$DATADIR/sites/basemodel.php.template", "$HOME/lib/base/${PROJECT_FILE}basemodel.class.php", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );
			writeTemplate( "$DATADIR/sites/basemodule.php.template", "$HOME/lib/base/${PROJECT_FILE}basemodule.class.php", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );
			writeTemplate( "$DATADIR/sites/baseuiedit.php.template", "$HOME/lib/base/${PROJECT_FILE}baseuiedit.class.php", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );
			...
			$aux .= '	</row>\n' ;
			$aux .= '</anydataset>\n' ;
			writeTemplate( $aux, "$HOME/data/anydataset/_db.anydata.xml", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );
			...
			$aux .= '	</row>\n' ;
			$aux .= '</anydataset>\n' ;
			writeTemplate( $aux, "$HOME/data/anydataset/_configemail.anydata.xml", array('/__PROJECT__/', '/__PROJECT_FILE__/'), array($PROJECT, $PROJECT_FILE ) );
...
function executeShell($cmd, $params)
{
	$final = "";
	$retorno = "";
...
function writeTemplate($template, $file, $pattern, $replace)
{
